...
from src.baish.storage import save_results_json, save_script
...
class TestStorage(unittest.TestCase):
    ...
    def test_save_results_json(self):
        ...
        unique_id = "test123"
        script_path = (
            Path(self.temp_dir) / "scripts" / f"{date_str}_{unique_id}_script.sh"
        )

        results_path = save_results_json(
            results, script_path, date_str, unique_id, self.mock_config
        )
    ...
    def test_save_script_matches_results_id(self):
        ...
        results_path = save_results_json(
            {}, Path(script_path), date_str, unique_id, self.mock_config
        )
