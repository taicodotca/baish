...
from src.baish.script_analyzer import (analyze_chunks, analyze_script,
...
class TestScriptAnalyzer(unittest.TestCase):
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_yara_match(
        self, mock_detect, mock_yara, mock_security
    ):
        mock_detect.return_value = {"mime_type": "text/x-shellscript", "is_text": True}
        mock_security.return_value.arun.return_value = {
            "harm_score": 8,
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_large_content(
        self, mock_detect, mock_yara, mock_security
    ):
        mock_detect.return_value = {"mime_type": "text/x-shellscript", "is_text": True}
        mock_security.return_value.arun.return_value = {
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_with_cli_provider(
        self, mock_detect, mock_yara, mock_security
    ):
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_with_debug(
        self, mock_detect, mock_yara, mock_security
    ):
        mock_detect.return_value = {"mime_type": "text/x-shellscript", "is_text": True}
        mock_security.return_value.arun.return_value = {
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_invalid_response(
        self, mock_detect, mock_yara, mock_security
    ):
        mock_detect.return_value = {"mime_type": "text/x-shellscript", "is_text": True}
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_missing_config(
        self, mock_detect, mock_yara, mock_security
    ):
        mock_detect.return_value = {"mime_type": "text/x-shellscript", "is_text": True}
        mock_security.return_value.arun.return_value = {
    ...
    @patch("src.baish.script_analyzer.analyze_script_with_yara")
    @patch("src.baish.script_analyzer.detect_file_type")
    async def test_analyze_script_with_results_manager(
        self, mock_detect, mock_yara, mock_security
    ):
    ...
    def test_calculate_chunk_size(self):
        with (
            patch("src.baish.script_analyzer.MAP_PROMPT") as mock_prompt,
    ...
    async def test_analyze_chunks_empty_chunks(self):
        results = await analyze_chunks(
            [], "text/x-shellscript", self.mock_config, None, False
        )
    ...
    async def test_analyze_chunks_map_phase_error(self, mock_get_llm):
        ...
        results = await analyze_chunks(
            ["chunk"], "text/x-shellscript", self.mock_config, None, True
        )
        self.assertEqual(results[2], "Failed to analyze script chunks")
    ...
    async def test_analyze_script_markdown_file(self, mock_detect):
        mock_detect.return_value = {"mime_type": "text/markdown", "is_text": True}
    ...
    async def test_analyze_chunks_invalid_map_result(self):
        with patch("src.baish.script_analyzer.get_llm") as mock_get_llm:
            ...
            result = await analyze_chunks(
                ["chunk1"], "text/x-python", self.mock_config, None, True
            )
    ...
    async def test_analyze_chunks_reduce_invalid_format(self):
        with patch("src.baish.script_analyzer.get_llm") as mock_get_llm:
            ...
            self.assertEqual(
                ...
                str(
                    ValueError(
                        "Expected dict, got <class 'str'>: invalid reduce result"
                    )
                ),
            )
